...
@section('content')
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl">
            ...
            <div class="bg-gradient-to-r from-[#ea3882] to-[#d12670] p-6 rounded-t-xl">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('user.bookings.detail', $booking->id_booking) }}"
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <div class="bg-gray-50 border border-gray-200 rounded-lg px-4 py-3">
                            <p class="font-bold text-gray-800">Rp {{ number_format($hargaPerBulan, 0, ',', '.') }}</p>
                    ...
                    <div>
                        ...
                        <select name="lama_sewa" id="lama_sewa" required
                            ...
                            @for ($i = 1; $i <= 12; $i++)
                                <option value="{{ $i }}"
                                    {{ old('lama_sewa', $booking->lama_sewa) == $i ? 'selected' : '' }}>
                                    {{ $i }} Bulan
                                </option>
                            @endfor
                        </select>
                    ...
                    @php
                        // Hitung harga per bulan dari total harga saat ini
                    ...
                    <div>
                        ...
                        <div class="bg-[#ffe6e2] border-2 border-[#f5acca] rounded-lg px-4 py-3">
                            ...
                            <p class="text-xs text-gray-600 mt-1">
                                Total otomatis dihitung berdasarkan durasi sewa
                            </p>
                        </div>
                    </div>
    ...
    <script>
        function updateTotal() {
            ...
            const hargaPerBulan = {{ $hargaPerBulan }}; // harga per bulan asli
            ...
            if (lamaSewa) {
                ...
                endDate.setMonth(endDate.getMonth() + lamaSewa);
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            const lamaSewa = parseInt(document.getElementById('lama_sewa').value);
            const hargaPerBulan = {{ $hargaPerBulan }};
            const total = hargaPerBulan * lamaSewa;

            // Ambil lama sewa lama dari server
            const lamaSewaLama = {{ $booking->lama_sewa }};

            // Kalau durasi tidak berubah, jangan tampilkan perubahan harga
            if (lamaSewa === lamaSewaLama) {
                if (!confirm(`Anda tidak mengubah durasi sewa.\nTetap simpan perubahan lain?`)) {
                    e.preventDefault();
                }
            } else {
                if (!confirm(
                        `Anda yakin ingin mengubah durasi sewa?\n\nDari ${lamaSewaLama} → ${lamaSewa} bulan\nTotal baru: Rp ${total.toLocaleString('id-ID')}`
                    )) {
                    e.preventDefault();
                }
            }
