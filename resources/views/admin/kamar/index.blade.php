...
<head>
    ...
    <title>Kelola Kamar - Koztly</title>
...
<body class="bg-gray-50">
    ...
    <aside id="sidebar" class="fixed left-0 top-0 z-40 w-64 h-screen bg-white shadow-lg sidebar-bg">
        <div class="h-full px-3 py-4 overflow-y-auto relative">
            ...
            <ul class="space-y-2 font-medium">
                ...
                <li>
                    <a href="{{ route('admin.kamar.index') }}" class="flex items-center p-3 text-white bg-pink-500 rounded-lg hover:bg-pink-600 transition">
    ...
    <div id="main-content" class="ml-64 transition-all duration-300">
        ...
        <nav class="bg-blue-300 shadow-md fixed top-0 z-20 right-0 left-64 transition-all duration-300" id="navbar">
            <div class="px-6 py-4">
                <div class="flex items-center justify-between">
                    ...
                    <h2 class="text-xl font-semibold text-white">Kelola Kamar</h2>
        ...
        <main class="mt-20 p-6">
            ...
            @if(session('error'))
            <div id="alert-error" class="mb-6 p-4 rounded-lg bg-red-100 text-red-800 transition-opacity duration-700">
                {{ session('error') }}
            </div>
            @endif
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                ...
                <div class="bg-white rounded-2xl shadow p-4">
                    <h2 class="text-sm font-semibold text-gray-700 mb-4">Top 3 Kosan dengan jumlah kamar terbanyak</h2>
                    <div class="space-y-3">
                        @foreach ($topKosan as $kosan)
                        <div class="bg-gray-50 rounded-xl p-3 flex justify-between items-center border-l-4 border-pink-400 shadow-sm">
                            <div class="flex flex-col">
                                <p class="font-medium text-gray-700 text-sm">{{ $kosan->nama_kos }}</p>
                                <p class="text-xs text-gray-500">{{ $kosan->jumlah_kamar }} kamar</p>
                            </div>
            ...
            <form method="GET" action="{{ route('admin.kamar.index') }}" class="bg-white p-6 rounded-2xl shadow mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Cari Kamar</label>
                        <input type="text" name="search" placeholder="Nomor kamar..." value="{{ request('search') }}"
                               class="w-full border border-gray-300 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-pink-400 text-sm">
                    </div>
                    
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Kosan</label>
                        <select name="kosan" class="w-full border border-gray-300 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-pink-400 text-sm">
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full border border-gray-300 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-pink-400 text-sm">
                    ...
                    <div class="flex gap-2">
                        <button type="submit" class="bg-pink-500 text-white rounded-xl px-4 py-2 flex items-center gap-2 hover:bg-pink-600 text-sm flex-1 justify-center">
                            <i class="fas fa-search"></i> Filter
                        </button>
                        <a href="{{ route('admin.kamar.index') }}" class="bg-gray-100 text-gray-700 rounded-xl px-4 py-2 flex items-center gap-2 hover:bg-gray-200 text-sm flex-1 justify-center">
            ...
            <div class="bg-white rounded-2xl shadow p-6">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button id="btnTambah" 
                       class="bg-blue-400 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-blue-500 text-sm">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-blue-200">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-sm font-medium text-gray-700">Nomor Kamar</th>
                                <th class="px-4 py-3 text-left text-sm font-medium text-gray-700">Kosan</th>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 flex items-center gap-2 text-sm text-gray-700">
                                <div class="bg-pink-100 text-pink-500 w-6 h-6 flex items-center justify-center rounded-full">
                                    <i class="fas fa-bed text-xs"></i>
                                </div>
                                {{ $k->nomor_kamar }}
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-700">{{ $k->kosan->nama_kos ?? '-' }}</td>
                        ...
                        <button class="bg-blue-500 text-white px-3 py-1 rounded-lg hover:bg-blue-600 flex items-center gap-1 btn-detail text-xs"
                        ...
                        data-booking='@json($bookingData)'><i class="fas fa-info-circle"></i> Detail</button>
                                ...
                                <!-- Tombol Edit -->
                                 ...
                                 class="bg-yellow-500 text-white px-3 py-1 rounded-lg hover:bg-yellow-600 flex items-center gap-1 text-xs">
                                 <i class="fas fa-edit"></i> Edit</button>

                                <!-- Hapus -->
                                <form method="POST" action="{{ route('admin.kamar.destroy', $k->id_kamar) }}" onsubmit="return confirm('Yakin ingin menghapus kamar ini?');" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bg-red-500 text-white px-3 py-1 rounded-lg hover:bg-red-600 flex items-center gap-1 text-xs">
                        ...
                        <tr>
                            <td colspan="6" class="py-12 text-center">
                                <div class="flex flex-col items-center justify-center text-gray-400 gap-2">
                                    <i class="fas fa-door-open text-4xl text-pink-500"></i>
    ...
    <script>
       ...
       document.querySelectorAll('.btn-detail').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const container = document.getElementById('detail_riwayat');
