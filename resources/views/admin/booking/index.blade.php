    ...
    <aside id="sidebar" class="sidebar-full fixed left-0 top-0 z-40 h-screen bg-white shadow-lg sidebar-bg transition-all duration-300">
        <div class="h-full px-3 py-4 overflow-y-auto relative">
            ...
            <ul class="space-y-2 font-medium">
                <li>
                    <a href="{{ route('admin.dashboard') }}" class="flex items-center p-3 text-gray-700 rounded-lg hover:bg-pink-50 hover:text-pink-600 transition" title="Dashboard">
        ...
        <main class="mt-20 p-6">
            ...
            <div class="flex justify-between mb-6 gap-2">
                <div class="flex gap-2">
                    <a href="{{ route('admin.booking.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-700 px-4 py-2 rounded-lg flex items-center">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <select id="sortSelect" class="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-pink-400">
            ...
            <div class="overflow-x-auto bg-white rounded-xl shadow p-4 border-l-4 border-pink-400">
                <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">
                                ...
                                @if(request('sort', 'terbaru') == 'terbaru')
                                    <i class="fas fa-arrow-down ml-1 text-xs text-pink-500"></i>
                                @else
                                    <i class="fas fa-arrow-up ml-1 text-xs text-pink-500"></i>
                                @endif
                            </th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2 capitalize">
                                    <span class="px-2 py-1 rounded-full text-xs font-medium 
                                        @if($booking->status_sewa == 'menunggu') bg-yellow-100 text-yellow-800
                                        @elseif($booking->status_sewa == 'aktif') bg-green-100 text-green-800
                                        @elseif($booking->status_sewa == 'selesai') bg-gray-100 text-gray-800
                                        @elseif($booking->status_sewa == 'batal') bg-red-100 text-red-800
                                        @endif">
                                        {{ $booking->status_sewa }}
                                    </span>
                                </td>
                                ...
                                <td class="px-4 py-2 flex gap-2">
                                    @if($booking->status_sewa == 'menunggu')
                                        ...
                                        <form action="{{ route('admin.booking.update', $booking->id_booking) }}" method="POST" class="cancel-form">
                                            ...
                                            <button type="button" class="cancel-btn bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-lg text-sm">
                                                Batal
                                            </button>
                                        </form>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-4 py-2 text-center text-gray-500">Tidak ada booking ditemukan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                <div id="modal-{{ $booking->id_booking }}" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
                    <div class="bg-white rounded-xl shadow-lg w-11/12 md:w-1/2 p-6 relative">
                        ...
                        <ul class="text-gray-700 space-y-2">
                            ...
                            <li><strong>Harga:</strong> Rp {{ number_format($booking->harga ?? 0, 0, ',', '.') }}</li>
                            <li><strong>Lama Sewa:</strong> {{ $booking->lama_sewa }} bulan</li>
                            ...
                            <li><strong>Gender User:</strong> {{ optional($booking->user)->gender ?? '-' }}</li>
                            <li><strong>Jumlah Penghuni:</strong> {{ $booking->jumlah_penghuni ?? '-' }}</li>
                            <li><strong>Catatan:</strong> {{ $booking->catatan ?? '-' }}</li>
                            ...
                            <li><strong>Dibuat:</strong> {{ \Carbon\Carbon::parse($booking->created_at)->format('d M Y H:i') }}</li>
                            <li><strong>Diubah:</strong> {{ \Carbon\Carbon::parse($booking->updated_at)->format('d M Y H:i') }}</li>
                        </ul>
